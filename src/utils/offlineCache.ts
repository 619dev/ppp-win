...
export function writeOfflineData<T>(key: string, value: T): void {
  ...
  catch (error) { console.warn('[OfflineCache] Could not persist data:', key, error) }
...
function collectMediaUrls(value: unknown, urls: Set<string>, depth = 0): void {
  ...
  if (typeof value === 'string') {
    if (/^(https?:|blob:)/i.test(value) || value.startsWith('/uploads/') || value.startsWith('/media/')) urls.add(value)
    return
  }
